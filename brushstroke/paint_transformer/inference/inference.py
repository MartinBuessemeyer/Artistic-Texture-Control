...
def save_img(img, output_path):
    result = Image.fromarray((img.data.cpu().numpy().transpose((1, 2, 0)).squeeze() * 255).astype(np.uint8))
    result.save(output_path)


def param2stroke(param, H, W, meta_brushes, use_erosion_dialtion=False):
    """
...
def param2img_parallel(param, decision, meta_brushes, cur_canvas, use_erosion_dialtion=False):
    """
        Input stroke parameters and decisions for each patch, meta brushes, current canvas, frame directory,
        and whether there is a border (if intermediate painting results are required).
        Output the painting results of adding the corresponding strokes on the current canvas.
    ...
    valid_foregrounds, valid_alphas = param2stroke(param[decision, :], patch_size_y, patch_size_x, meta_brushes,
                                                   use_erosion_dialtion=use_erosion_dialtion)
    ...
    def partial_render(this_canvas, patch_coord_y, patch_coord_x):
        ...
        canvas_patch = F.unfold(this_canvas, (patch_size_y, patch_size_x),
                                stride=(patch_size_y // 2, patch_size_x // 2))
        # canvas_patch: b, 3 * py * px, h * w
        canvas_patch = canvas_patch.view(b, num_color_channels, patch_size_y, patch_size_x, h, w).contiguous()
        canvas_patch = canvas_patch.permute(0, 4, 5, 1, 2, 3).contiguous()
        # canvas_patch: b, h, w, 3, py, px
        selected_canvas_patch = canvas_patch[:, patch_coord_y, patch_coord_x, :, :, :]
        ...
        this_canvas = this_canvas.view(b, num_color_channels, h_half * patch_size_y, w_half * patch_size_x).contiguous()
        # this_canvas: b, 3, h_half * py, w_half * px
        return this_canvas
...
def main(input_path, model_path, output_dir, need_animation=False, resize_h=None, resize_w=None, serial=False,
    ...
    net_g = network.Painter(5, stroke_num, 256, 8, 3, 3, input_nc=(1 if grayscale else 3)).to(device)
    ...
    brush_large_vertical = read_img(
        './brushstroke/paint_transformer/inference/brush/brush_large_vertical.png',
        'L').to(device)
    ...
    with torch.no_grad():
        original_img = read_img(input_path, 'L' if grayscale else 'RGB', resize_h, resize_w).to(device)
        batch_size, channel_size, original_h, original_w = original_img.shape
        ...
        for layer in range(0, K + 1):
            if layer > 3:
                ...
                img_patch = img_patch.permute(0, 2, 1).contiguous().view(-1, channel_size, patch_size,
                                                                         patch_size).contiguous()
                result_patch = result_patch.permute(0, 2, 1).contiguous().view(
                    -1, channel_size, patch_size, patch_size).contiguous()
                ...
                img_temp = img_patch.unsqueeze(1).contiguous().repeat(1, stroke_num, 1, 1, 1).view(
                    img_patch.shape[0] * stroke_num, channel_size, patch_size, patch_size).contiguous()
                color = F.grid_sample(img_temp, 2 * grid - 1, align_corners=False).view(
                    img_patch.shape[0], stroke_num, channel_size).contiguous()
                ...
                param = stroke_param.view(1, patch_num, patch_num, stroke_num, stroke_param.shape[-1]).contiguous()
                ...
                if serial:
                    raise NotImplementedError('Code needs adaption before use!')
        ...
        # img_patch, result_patch: b, 3 * output_size * output_size, h * w
        img_patch = img_patch.permute(0, 2, 1).contiguous().view(-1, channel_size, patch_size, patch_size).contiguous()
        result_patch = result_patch.permute(0, 2, 1).contiguous().view(-1, channel_size, patch_size,
                                                                       patch_size).contiguous()
        shape_param, stroke_decision = net_g(img_patch, result_patch)
        ...
        # stroke_param: b * h * w, stroke_per_patch, param_per_stroke
        # stroke_decision: b * h * w, stroke_per_patch, 1
        param = stroke_param.view(1, h, w, stroke_num, stroke_param.shape[-1]).contiguous()
        ...
        if serial:
            ...
            final_result = param2img_parallel(param, decision, meta_brushes, final_result, use_erosion_dialtion=False)
...
if __name__ == '__main__':
    main(input_path='./tmp/step_43000_contour.png',
         # model_path='./brushstroke/paint_transformer/inference/model.pth',
         model_path='./brushstroke/paint_transformer/train/checkpoints/painter_grayscale/latest_net_g.pth',
         output_dir='tmp/brushstroke-spike/',

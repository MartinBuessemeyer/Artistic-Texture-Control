...
class PainterModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_recall', type=float, default=10.0,
                            help='weight of recall for stroke decision loss')
        return parser
    ...
    def __init__(self, opt):
        ...
        self.num_color_channels = opt.input_nc
        num_features = 6  # xc, yc, w, h, theta, R0, G0, B0, R2, G2, B2, A
        num_features += self.num_color_channels * 2  # Add colors
        self.d = num_features  # vorher 12
        ...
        brush_large_vertical = read_img('brush/rectangle_large.png', 'L').to(self.device)
        brush_large_horizontal = read_img('brush/rectangle_large.png', 'L').to(self.device)
        self.meta_brushes = torch.cat(
            [brush_large_vertical, brush_large_horizontal], dim=0)
        net_g = networks.Painter(self.d_shape, opt.used_strokes, opt.ngf,
                                 n_enc_layers=opt.num_blocks, n_dec_layers=opt.num_blocks, input_nc=opt.input_nc)
    ...
    def param2stroke(self, param, H, W):
        ...
        color0 = param[:, -(1 + 2 * self.num_color_channels):- (1 + self.num_color_channels), ...] \
            .view(-1, self.num_color_channels, 1)
        color2 = param[:, -(1 + self.num_color_channels):-1, ...].view(-1, self.num_color_channels, 1)
        ...
        alphas = torch.cat([brush] * self.num_color_channels, dim=1)
        alphas = (alphas > 0).float()
        t = torch.arange(0, brush.shape[2], device=param.device).view(1, 1, -1) / brush.shape[2]
        color_map = color0 * (1 - t) + color2 * t
    ...
    def set_input(self, input_dict):
        ...
        with torch.no_grad():
            ...
            old_param[:, :, -(1 + self.num_color_channels):-1] = \
                old_param[:, :, -(1 + 2 * self.num_color_channels):-(1 + self.num_color_channels)]
            ...
            foregrounds = foregrounds.view(self.opt.batch_size // 4, self.opt.used_strokes, self.num_color_channels,
                                           self.patch_size * 2,
                                           self.patch_size * 2).contiguous()
            alphas = alphas.view(self.opt.batch_size // 4, self.opt.used_strokes, self.num_color_channels,
                                 self.patch_size * 2,
                                 self.patch_size * 2).contiguous()
            old = torch.zeros(self.opt.batch_size // 4, self.num_color_channels, self.patch_size * 2,
                              self.patch_size * 2, device=self.device)
            for i in range(self.opt.used_strokes):
                foreground = foregrounds[:, i, :, :, :]
                alpha = alphas[:, i, :, :, :]
                old = foreground * alpha + old * (1 - alpha)
            old = old.view(self.opt.batch_size // 4, self.num_color_channels, 2, self.patch_size, 2,
                           self.patch_size).contiguous()
            old = old.permute(0, 2, 4, 1, 3, 5).contiguous()
            self.old = old.view(self.opt.batch_size, self.num_color_channels, self.patch_size,
                                self.patch_size).contiguous()

            gt_param = torch.rand(self.opt.batch_size, self.opt.used_strokes, self.d, device=self.device)
            gt_param[:, :, :4] = gt_param[:, :, :4] * 0.5 + 0.2
            gt_param[:, :, -(1 + self.num_color_channels):-1] = \
                gt_param[:, :, -(1 + 2 * self.num_color_channels):-(1 + self.num_color_channels)]
            ...
            foregrounds = morphology.Dilation2d(m=1)(foregrounds)
            alphas = morphology.Erosion2d(m=1)(alphas)
            foregrounds = foregrounds.view(self.opt.batch_size, self.opt.used_strokes, self.num_color_channels,
                                           self.patch_size,
                                           self.patch_size).contiguous()
            alphas = alphas.view(self.opt.batch_size, self.opt.used_strokes, self.num_color_channels, self.patch_size,
                                 self.patch_size).contiguous()
    ...
    def forward(self):
        ...
        foregrounds = foregrounds.view(-1, self.opt.used_strokes, self.num_color_channels, self.patch_size,
                                       self.patch_size)
        alphas = alphas.view(-1, self.opt.used_strokes, self.num_color_channels, self.patch_size, self.patch_size)
        # foreground, alpha: b, stroke_per_patch, 3, output_size, output_size

...
class Painter(nn.Module):
    ...
    def forward(self, img, canvas):
        ...
        img_temp = img.unsqueeze(1).contiguous().repeat(1, s, 1, 1, 1).view(b * s, c, H, W).contiguous()
        color = nn.functional.grid_sample(img_temp, 2 * grid - 1, align_corners=False).view(b, s, c).contiguous()

...
class ArbitraryStyleEffect(EffectBase):
    def __init__(self, contours_on_content=False, content_img=None):
        super().__init__(arbitrary_style_vp_ranges)
        # locals
        dim_kernsize = 10
        self.contours_on_content = contours_on_content
    ...
    def forward_effect(self, x, visual_parameters):
        ...
        _ = self.vpd.select_parameter(visual_parameters, "bump_phong_shininess")
        ...
        xdog_input = x if not self.contours_on_content else self.content_img
        stylized_image, xdog_intermediate = self.contours_part(xdog_input, stylized_image, i,
                                                               contour,
                                                               contour_opacity)
    ...
    def contours_part(self, image, stylized_image, i,
        ...
        xdog_epsilon = torch.tensor(0.5, device=image.device)
    ...
    def smoothing_part_flow_aligned(self, image, flow_aligned_bilateral_sigma_d_1, flow_aligned_bilateral_sigma_r_1,
                                    flow_aligned_bilateral_sigma_d_2, flow_aligned_bilateral_sigma_r_2):
        structure_tensor_sigma = torch.tensor(8.0, device=image.device)
        tf_1_gauss_sigma = torch.tensor(0.5, device=image.device)

        rgb_2_lab_1 = self.run(self.rgb_to_lab, image)
        tf_1, structure_tensor_1 = self.tf_map(rgb_2_lab_1, structure_tensor_sigma, tf_1_gauss_sigma)
        bilateral_color_intermediate = self.run(self.bilateralPass0, rgb_2_lab_1, tf_1,
                                                flow_aligned_bilateral_sigma_d_1,
                                                flow_aligned_bilateral_sigma_r_1)
        bilateral_color = self.run(self.bilateralPass1, bilateral_color_intermediate, tf_1,
                                   flow_aligned_bilateral_sigma_d_2, flow_aligned_bilateral_sigma_r_2)
        return self.lab_to_rgb(bilateral_color)

...
from helpers.hist_metrics import DeepHistLoss
...
def optimize(result, content, style, scale, content_weight, lr, extractor, use_hist=False):
    # torch.autograd.set_detect_anomaly(True)
    result_pyramid = make_laplace_pyramid(result, 5)
    result_pyramid = [l.data.requires_grad_() for l in result_pyramid]

    if use_hist:
        hist_loss = DeepHistLoss().to(device)
    ...
    for it in range(opt_iter):
        ...
        if use_hist:
            loss += args.lambda_hist * hist_loss(stylized, content)[0]
        loss.backward()
        optimizer.step()
    return stylized


def strotss(content_pil, style_pil, content_weight=1.0 * 16.0, device='cuda:0', space='uniform', use_hist=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-hist", action="store_true", help="use histogram loss")
    parser.add_argument('--lambda_hist', type=float, default=1.0, help='histogram loss parameter weight')

...
drawing_mode = st.sidebar.selectbox(
    "Effect:", ("xdog", "toon", "watercolor", "oilpaint")
)
...
effect_and_vps_match = len(param_set) == vp.shape[1]
if not effect_and_vps_match:
    st.error(
        f'Currently have {vp.shape[1]} different visual parameters but effect {drawing_mode} expects {len(param_set)}')
...
img = np.array(img_org.resize((basewidth, hsize), Image.Resampling.LANCZOS)) / (255.0 * 2.0) + 0.5
img[:, :, 0] = 0
...
if canvas_result.image_data is not None:
    abc = np_to_torch(canvas_result.image_data.astype(float)).sum(dim=1, keepdim=True).cuda()
